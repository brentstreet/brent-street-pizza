...
export default function MenuItemCard({ item, onAdd }: MenuItemCardProps) {
  ...
  return (
    <div className="group relative bg-[#2b1200] rounded-[14px] border border-white/5 overflow-hidden flex flex-col hover:border-[#C0392B]/30 hover:-translate-y-1.5 hover:shadow-[0_20px_50px_-10px_rgba(192,57,43,0.25)] transition-all duration-400 h-full">
      ...
      <div className="p-5 flex flex-col flex-grow gap-3">
        ...
        <button
          onClick={() => onAdd(item)}
          id={`card-add-${item.id}`}
          className="mt-auto flex items-center justify-between bg-white/5 hover:bg-[#C0392B] border border-white/10 hover:border-[#C0392B] text-white font-barlow text-[13px] font-700 uppercase tracking-wider px-4 py-3 rounded-[8px] transition-all duration-300 group/btn"
        >
