            ...
            items.map(item => (
              ...
              >
                ...
                <div className="flex-grow min-w-0">
                  ...
                  <div className="mb-2.5 space-y-0.5">
                    {/* NEW: Render standard product variant */}
                    {item.variant && <p className="font-inter text-[11px] text-[#C8201A] font-bold leading-snug">Option: {item.variant}</p>}
                    ...
                    {/* NEW: Render nested combo/deal variants (e.g. Can 375ml -> Sprite) */}
                    {item.selectedDealItems && item.selectedDealItems.length > 0 && (
                      <div className="mt-1 flex flex-col gap-0.5">
                        {item.selectedDealItems.map((dealItem: any, idx: number) => (
                           <p key={idx} className="font-inter text-[11px] text-[#555555] leading-snug">
                             • {dealItem.name} {dealItem.variant ? <span className="text-[#C8201A] font-medium">({dealItem.variant})</span> : ''}
                           </p>
                        ))}
                      </div>
                    )}
                  ...
                  <div className="flex items-center gap-0.5 bg-[#FDF8F2] rounded-full w-fit border border-[#E8D8C8] px-1.5 py-1 mt-1">
                    <button onClick={() => onDecrement(item.id)} className="w-6 h-6 flex items-center justify-center text-[#555555] hover:text-[#C8201A] transition-colors active:scale-90">
                      {item.quantity === 1 ? <Trash2 className="w-3 h-3" /> : <Minus className="w-3 h-3" />}
                    </button>
                    <span className="font-bebas text-[17px] text-[#2B2B2B] w-6 text-center leading-none">{item.quantity}</span>
