...
const Footer: React.FC = () => {
  const { sectionContent, loading } = useSectionContent('footer');
  const { content } = useContent();
  const global = content.global || {};

  if (loading) return <footer className="bg-[#1A1A1A] h-20" />;
        ...
        <div className="container-custom py-12 flex flex-col md:flex-row items-center justify-between gap-8">
          <div>
            <h3 className="font-bebas text-[36px] md:text-[44px] tracking-wider text-[#CCCCCC] leading-none">
              {sectionContent.cta_title || 'Ready to Order Direct?'}
            </h3>
          </div>
      ...
      <div className="container-custom pt-16 pb-10 relative z-10">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 md:gap-8 mb-14">
          ...
          <div className="flex flex-col gap-5">
            <div>
              <h2 className="font-bebas text-[30px] tracking-widest text-[#CCCCCC] leading-none">{sectionContent.brand_title_1 || 'BRENT STREET'}</h2>
              <h2 className="font-bebas text-[30px] tracking-widest text-[#CCCCCC] leading-none">{sectionContent.brand_title_2 || 'PIZZA'}</h2>
            </div>
            <p className="font-inter text-[13px] text-[#CCCCCC] leading-relaxed max-w-[200px]">
              {sectionContent.brand_description || 'Brent Street Pizza is your local go-to for hot, fresh pizza made to order. Premium ingredients. Big flavour.'}
            </p>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex items-start gap-2.5">
              ...
              <p className="font-inter text-[13px] text-[#CCCCCC] leading-relaxed">
                {global.address_line_1 || '2 Brent Street,'}<br />
                {global.address_line_2 || 'Glenorchy TAS 7010'}
              </p>
            </div>
            <a href={`tel:${global.phone || '0362724004'}`} className="flex items-center gap-2.5 group">
              <Phone className="w-4 h-4 text-[#CCCCCC] flex-shrink-0" />
              <span className="font-bebas text-[22px] tracking-wider text-[#CCCCCC] group-hover:text-[#C8201A] transition-colors">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="bg-[#2a2a2a] p-4 rounded-xl border border-white/10">
              {(sectionContent.trading_hours || []).map((th: any) => (
                <div key={th.label} className="mb-4 last:mb-0">
                  <p className="font-barlow text-[11px] font-700 uppercase tracking-wider text-[#c9922a] mb-1">{th.label}</p>
                  {Array.isArray(th.value) ? (
                    th.value.map((v: string) => <p key={v} className="font-inter text-[13px] text-white">{v}</p>)
                  ) : (
                    <p className="font-inter text-[13px] text-white">{th.value}</p>
                  )}
                </div>
              ))}
            </div>
