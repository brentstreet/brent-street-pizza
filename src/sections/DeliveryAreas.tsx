...
const DeliveryAreas: React.FC = () => {
  ...
  // Import cart context to update global order type state
  ...
  const handleDeliveryClick = () => {
    // 1. If your CartContext has a state setter for order type, call it
    if (cart?.setOrderType) cart.setOrderType('delivery');
    if (cart?.setOrderMethod) cart.setOrderMethod('delivery');
    
    // 2. Persist to localStorage so the Cart/Checkout pages automatically read it on load
