...
// const Hero: React.FC = () => {
//   const { sectionContent, loading } = useSectionContent('hero');
...
//         style={{ backgroundImage: `url(${sectionContent.image || '/heropic.jpeg'})` }}
//       />
...
//             >
//               {sectionContent.subtitle || 'Hot & Fresh'}
//           </h3>
//             <h1
...
//             >
//                {sectionContent.description || 'Made to order.'}
//             </p>
...
//             >
//               <Link
//                 to={sectionContent.cta_pickup_link || "/menu"}
...
//               >
//                 {sectionContent.cta_pickup_text || 'ORDER PICKUP'} <span className="text-xl">›</span>
//               </Link>
//               <Link
//                 to={sectionContent.cta_delivery_link || "/menu?tab=delivery"}
//                 id="hero-order-delivery"
//                 className="bg-gradient-to-b from-[#dc2626] to-[#991b1b] hover:from-[#ef4444] hover:to-[#b91c1c] text-white font-barlow font-800 text-[14px] sm:text-[16px] md:text-[18px] tracking-widest px-6 sm:px-6 md:px-8 py-3.5 rounded shadow-[0_4px_0_#7f1d1d,0_8px_15px_rgba(0,0,0,0.5)] flex items-center justify-center gap-2 transition-all duration-300 whitespace-nowrap active:translate-y-1 active:shadow-[0_0px_0_#7f1d1d] uppercase border border-[#f87171]/20"
//               >
//                 {sectionContent.cta_delivery_text || 'ORDER DELIVERY'} <span className="text-xl">›</span>
//               </Link>
//             </div>
//           </div>
//         </div>
...
const Hero: React.FC = () => {
  const { sectionContent, loading } = useSectionContent('hero');
  const { setOrderType } = useCart(); // Access setOrderType
  ...
  const handleOrderPickup = () => {
    setOrderType('pickup');
    navigate(sectionContent.cta_pickup_link || "/menu");
  };

  const handleOrderDelivery = () => {
    setOrderType('delivery');
    navigate(sectionContent.cta_delivery_link || "/menu?tab=delivery");
  };
