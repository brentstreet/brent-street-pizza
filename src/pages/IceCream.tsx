...
export default function IceCream() {
  ...
  const handleCustomAdd = (customs: any) => {
    // Look for the product in the DB, but provide a robust fallback if it doesn't exist
    const iceCreamProduct = products.find((p: any) => 
          ...
          <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-6">
            {(sectionContent.specials || []).map((item: any) => (
              <SpecialIceCreamCard 
                ...
                item={item} 
                onAddToCart={(it) => {
                  const p = products.find((prod: any) => prod.name === it.name)
                    || products.find((prod: any) => prod.id.includes(it.id))
                    || {
                        ...it,
                        id: `ice-cream-${it.id}`,
                        categoryId: 'cat-ice-cream',
